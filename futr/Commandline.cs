...
public class Commandline
{
    ...
    protected HandlerMap Handlers = new(StringComparer.OrdinalIgnoreCase);
    protected FormatterList Formatters = new();
    protected ArgumentFormatterList ArgumentFormatters = new();
    ...
    public Commandline()
    {
        Handlers.Add("Echo", new Handler { Name = "Echo", Function = Echo, Role = Role.CommandlinePublic.ToString(), ImmediateExecute = true, Description = "Return all arguments", ArgumentList = ArgumentListType.Tokens, Arguments = new ArgumentDescriptionList { { "arg1", "first argument" }, { "...", "more arguments" } } });
        Handlers.Add("Dev_TestTable", new Handler { Name = "Dev_TestTable", Function = Dev_TestTable, ImmediateExecute = true, Role = Role.CommandlineAdmin.ToString(), Description = "Full table example" });
        Handlers.Add("Dev_Exception", new Handler { Name = "Dev_Exception", Function = Dev_Exception, ImmediateExecute = true, Role = Role.CommandlineAdmin.ToString(), Description = "Throw exception" });
        Handlers.Add("Dev_null", new Handler { Name = "Dev_null", Function = Dev_null, ImmediateExecute = true, Role = Role.CommandlineAdmin.ToString(), Description = "Do nothing, return null" });
        Handlers.Add("var", new Handler { Name = "var", Function = GetSetVar, Role = Role.CommandlinePublic.ToString(), ImmediateExecute = false, Description = "Assign or use variable", ArgumentList = ArgumentListType.Tokens, Arguments = new ArgumentDescriptionList { ["Name[=Value]"] = "Name=Value assigns variable value to name, Name only returns variable value", } });
        Handlers.Add("//", new Handler { Name = "//", Function = Comment, Role = Role.CommandlinePublic.ToString(), ImmediateExecute = false, Description = "Ignored and copied to output", ArgumentList = ArgumentListType.Tokens, Arguments = new ArgumentDescriptionList { ["Comment"] = "Comment line", } });
        Handlers.Add("Dev_Sleep", new Handler { Name = nameof(Dev_Sleep), Function = Dev_Sleep, Role = Role.CommandlinePublic.ToString(), ImmediateExecute = true, Description = "Sleep n sec", ArgumentList = ArgumentListType.Tokens, Arguments = new ArgumentDescriptionList { ["2"] = "Seconds", } });

        Formatters.Add(FormatStringList);
    ...
    public class Table
    {
        ...
        public OptionList Options { get { return _options ??= new OptionList(); } }

        public class Row : List<string> { }
        public class RowList : List<Row> { }
        private RowList _grid = null;
        public RowList Grid { get { return _grid ??= new RowList(); } }
    }
        ...
        foreach (var line in lines) {
            ...
            if (line.Count > 0) {
                try {
                    ...
                    var text = line.Source + ": " + string.Join(" | ", ex.GetMessages());
                    if (lines.Count == 1) {
                        throw new Exception("Error: " + text);
                    } else {
                        throw new Exception("Error at line " + lineCount + ": " + text);
                    }
                }
            }
        }
    ...
    private object Dev_Sleep(Arglist args)
    {
        args.Next("cmd");
        var sec = int.Parse(args.Next("Seconds", "2"));
    ...
    {
        if (o is not string) { return null; }
        return o as string;
    }

    public string FormatStringList(object o)
    {
        if (o is IEnumerable<string> l) {
            return string.Join("\n", l.Select(i => i + "<br/>"));
        }
        return o as string;
    }
    ...
    {
        if (o is not VariableAssignment) { return null; }
    ...
    public string FormatHtmlAttributes(Dictionary<string, string> htmlAttributes)
    {
        if (htmlAttributes != null) {
            return htmlAttributes.Aggregate(new StringBuilder(), (sb, x) => sb.Append(" " + WebUtility.HtmlEncode(x.Key) + "=" + "\"" + WebUtility.HtmlEncode(x.Value) + "\""), sbAttr => sbAttr.ToString());
        }
        return "";
    }
    ...
    public string CommandInsertLink(string method, IEnumerable<string> args, string text, Dictionary<string, string> htmlAttributes = null)
    {
        ...
        sb.Append(WebUtility.HtmlEncode(method + " " + string.Join(" ", args)));
